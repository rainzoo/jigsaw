...
PARAMS_XML = """<c:params xmlns:c="http://s.opencalais.com/1/pred/" 
xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
<c:processingDirectives %s> </c:processingDirectives> <c:userDirectives %s> 
</c:userDirectives> <c:externalMetadata %s> </c:externalMetadata> </c:params>
"""
...
class AppURLopener(urllib.FancyURLopener):
    version = """Mozilla/5.0 (X11; U; Linux x86_64; en-US; rv:1.9.0.5)
     Gecko/2008121623 Ubuntu/8.10 (intrepid)Firefox/3.0.5" # Lie shamelessly to Wikipedia.
     """
...
class Calais():
    ...
    Use the analyze() and analyze_url() methods, which return CalaisResponse objects.  
    """
    api_key = None
    processing_directives = {"contentType":"TEXT/RAW", "outputFormat":"application/json",
                              "reltagBaseURL":None, "calculateRelevanceScore":"true",
                              "enableMetadataType":"GenericRelations,SocialTags", "discardMetadata":None,
    ...
    def _get_params_XML(self):
        return PARAMS_XML % (" ".join('c:%s="%s"' % (k, escape(v)) 
                                      for (k, v) in self.processing_directives.items() if v),
                              " ".join('c:%s="%s"' % (k, escape(v)) 
                                       for (k, v) in self.user_directives.items() if v),
                              " ".join('c:%s="%s"' % (k, escape(v)) 
                                       for (k, v) in self.external_metadata.items() if v))
...
class CalaisResponse():
    ...
    def __init__(self, raw_result):
        try:
            self.raw_response = json.load(StringIO(raw_result.decode('utf-8', "[removed]")),
                                          encoding="utf-8")
        except:
    ...
    def get_entities(self):
        ...
        for item in self.entities:
            e += "{0} {1} {2}".format(item['_type'], item['name'],
                                     item['relevance'])
        return e 
    ...
    Adding helpers
    """
    def get_json_entities(self):
        response = []
        for e in self.simplified_response['entities']:
            entity = {}
            try:
                entity['name'] = e['name']
                entity['type'] = e['_type']
    ...
    def get_json_rels(self):
        response = []
        for e in self.simplified_response['relations']:
            entity = {}
            try:
                entity['name'] = e['name']
                entity['type'] = e['_type']
